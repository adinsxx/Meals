...
public class Main {
    ...
    public Main() {
        ...
        while ((line = indata.fileReadLine()) != null) {
            mealDetails = line.split(",");
            calories = Integer.parseInt(mealDetails[2]);
            if (max < calories){
                max = calories;
            }
            if (min > calories){
            ...
            if(oldMealType.equals(mealDetails[0]) || oldMealType.equals("NA")){
                count++;
                totalCalories += calories;
            }
            else {
                System.out.println(oldMealType + " avg calories " + (totalCalories/count) + " max " + max + " min " + min);
                totalCalories = calories;
                count=1;
                max=0;
